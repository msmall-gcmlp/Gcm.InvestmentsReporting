...
class RunTwrorIndustry():
    ...
    def append_deal_attributes(self, df):
        # tickers = str(list(df.Owner.unique()))
        # tickers = tickers.replace('[', '(')
        # tickers = tickers.replace(']', ')')
        def my_dao_operation(dao, params):
            # raw = """select os.Ticker OsTicker, h.ReportingName, d.*  from entitydata.OperationalSeries os
            ...
            #         where os.Ticker in """ + tickers
            raw = """select distinct os.Ticker OsTicker, h.ReportingName, hrpting.PredominantSector, d.*  from entitydata.OperationalSeries os
                    left join entitydata.Investment i on os.MasterId = i.OperationalSeriesId
                    left join entitydata.Deal d on i.DealId = d.MasterId
                    left join entitydata.Holding h on i.HoldingId = h.MasterId
                    left join entitydata.Holding hrpting on h.ReportingMasterId = hrpting.MasterId
                    where os.Ticker in """ + self._os_series
            df = pd.read_sql(
                raw,
        ...
        attrib.PredominantSector = np.where(attrib.Name == 'Epic Midstream', 'Power Generation - Conventional', attrib.PredominantSector)
        attrib.PredominantSector = np.where(attrib.Name == 'Colossus', 'FUNDS-Generalist',
                                            attrib.PredominantSector)
        attrib.PredominantSector = np.where(attrib.Name == 'Project Matrix', 'FUNDS-Generalist',
                                            attrib.PredominantSector)

        rslt = df.merge(attrib, how='left', left_on=['Owner', 'Investment'], right_on=['OsTicker', 'ReportingName'])
        assert(len(rslt) == len(df))
        return rslt

    def get_cf_ilevel(self, date):
        # def my_dao_operation(dao, params):
        #     owner_list = self._os_series.replace('(', '').replace(')', '').replace("'",'').replace("'",'')
        #     raw = "EXEC iLevel.GetCashFlowData @OwnerName ='" + str(owner_list) + "', @Date='" + str(date) +"'"
        #     df = pd.read_sql(
        #         raw,
        #         dao.data_engine.session.bind,
        #     )
        ...
        df['TransactionType'] = np.where(df.TransactionType.str.contains('Contributions -'), 'Contributions', df.TransactionType)
        df['TransactionType'] = np.where(df.TransactionType.str.contains('Distributions -'), 'Distributions', df.TransactionType)
        df.rename(columns={'OwnerName': 'Owner',
                           'InvestmentName': 'Investment'}, inplace=True)
    ...
    def get_all_usd_cash_flows(self):
        def my_dao_operation(dao, params):
            raw = "select * from  [iLevel].[vExtendedCollapsedCashflows] where BaseCurrency='USD' and TransactionType in " \
        ...
        df['BaseAmount'] = np.where(df.TransactionType != 'Net Asset Value', df.BaseAmount * -1, df.BaseAmount)
        return df

    def get_cash_flows(self, date):
        def my_dao_operation(dao, params):
            raw = "select * from  [iLevel].vExtendedCollapsedCashflows where OwnerName in " + self._os_series + " and " \
                         "TransactionDate <= '" + str(date) +"' and TransactionType in " \
              "('Contributions - Investments and Expenses'," \
              "'Distributions - Recallable'," \
              "'Distributions - Return of Cost'," \
              "'Distributions - Gain/(Loss)'," \
    ...
    def get_last_day_of_prior_quarter(self, p_date):
        return dt.date(p_date.year, 3 * ((p_date.month - 1) // 3) + 1, 1) + dt.timedelta(days=-1)


    def get_last_day_of_the_quarter(self, p_date):
        quarter = (p_date.month - 1) // 3 + 1
        return dt.date(p_date.year + 3 * quarter // 12, 3 * quarter % 12 + 1, 1) + dt.timedelta(days=-1)
    ...
    def fill_forward_navs(self, nav_orig):
        ...
        nav_filled = nav_orig.pivot(index='TransactionDate',
                                    columns=['Owner', 'Investment'],
                                    values='BaseAmount').ffill().fillna(0)
        nav_filled = nav_filled.T.reset_index().melt(id_vars=['Owner', 'Investment'],
                                                   value_vars=None,
                                                   var_name='TransactionDate',
                                                   value_name='BaseAmount')
        nav_rslt = nav_filled.merge(nav_orig.drop(columns=['TransactionDate', 'BaseAmount']), how='left',
    ...
    def calc_tw_ror(self, df, group_cols, return_support_data=False):
        # fill forward NAVs
        # not used but we could
        # filled_navs = self.fill_forward_navs(df[df.TransactionType == 'Net Asset Value'])
        # df = pd.concat([df[df.TransactionType != 'Net Asset Value'], filled_navs]).sort_values('TransactionDate')

        max_nav_date = df[df.TransactionType == 'Net Asset Value'].groupby(['Owner', 'Investment']).\
            TransactionDate.max().reset_index().rename(columns={'TransactionDate':'MaxNavDate'})
        df = df.merge(max_nav_date, how='left', left_on=['Owner', 'Investment'], right_on=['Owner', 'Investment'])
        df = df[df.TransactionDate <= df.MaxNavDate]

        df = df.sort_values('TransactionDate')
        df['lastq'] = df.TransactionDate.apply(lambda row: self.get_last_day_of_prior_quarter(row))
        df['thisq'] = df.TransactionDate.apply(lambda row: self.get_last_day_of_the_quarter(row))
        ...
        df['Weight'] = np.where(df.TransactionType == 'Net Asset Value', 0, df.Weight)
        df['wAmount'] = df.Weight * df.BaseAmount

        # nav = df[df.TransactionType=='Net Asset Value'][['Owner', 'Investment', 'thisq', 'lastq', 'BaseAmount']].rename(columns={'BaseAmount': 'Nav'})
        nav = df[df.TransactionType=='Net Asset Value'].rename(columns={'BaseAmount': 'Nav'})
        nav = nav.merge(nav[self._attributes_needed + ['Owner', 'Investment', 'PredominantStrategy',
                                             'thisq', 'Nav']].
                        rename(columns={'Nav': 'PriorNav', 'thisq': 'lastq'}),
                 how='left', left_on=self._attributes_needed + ['Owner', 'Investment', 'PredominantStrategy',
                                             'lastq'], right_on=self._attributes_needed + ['Owner', 'Investment', 'PredominantStrategy',
                                             'lastq'])
        cf = df[df.TransactionType != 'Net Asset Value']

        data = pd.concat([nav, cf])

        # temp change to understand affect on top level ROR
        lowest_group = group_cols.copy()
        if 'Name' not in group_cols:
            lowest_group.extend(['Name'])

        data_sum_lowest = data[lowest_group + ['thisq', 'Nav', 'PriorNav', 'BaseAmount', 'wAmount']]. \
            groupby(lowest_group + ['thisq']).sum().reset_index()

        data_sum_lowest['EndingNavAdj'] = data_sum_lowest.Nav + data_sum_lowest.BaseAmount
        data_sum_lowest['PriorNavAdj'] = data_sum_lowest.PriorNav - data_sum_lowest.wAmount
        data_sum_lowest[['Nav', 'PriorNav', 'BaseAmount', 'wAmount', 'EndingNavAdj', 'PriorNavAdj']] = data_sum_lowest[['Nav', 'PriorNav', 'BaseAmount', 'wAmount', 'EndingNavAdj', 'PriorNavAdj']].fillna(0)
        ...
        data_sum = data_sum_lowest[group_cols + ['thisq', 'Nav', 'PriorNav', 'BaseAmount', 'wAmount', 'EndingNavAdj', 'PriorNavAdj']].\
            groupby(group_cols + ['thisq']).sum().reset_index()
        ...
        data_sum['Ror'] = (data_sum.EndingNavAdj - data_sum.PriorNav) / data_sum.PriorNavAdj
        ...
        navAdj = data_sum[['thisq', 'PriorNavAdj']].groupby(['thisq']).sum().reset_index().rename(columns={'PriorNavAdj': 'TotalPriorNav'})

        rslt = data_sum.merge(navAdj, how='left', left_on='thisq', right_on='thisq')
        rslt['Alloc'] = rslt.PriorNavAdj / rslt.TotalPriorNav
        rslt['Ctr'] = rslt.Alloc * rslt.Ror

        rslt['Name'] = rslt.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]), axis=1)
    ...
    def calc_irr(self, cf, group_cols=None, type='Gross'):
        ...
        if group_cols is None:
            irr = xirr(cf[["TransactionDate", "BaseAmount"]].groupby('TransactionDate').sum().reset_index())
        else:
            irr = cf.groupby(group_cols)[["TransactionDate", "BaseAmount"]].apply(xirr, silent=True)
            irr = irr.reset_index().rename(columns={0: type+'Irr'})
            irr['Name'] = irr.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]), axis=1)
        return irr

    def ann_return(self, return_df, trailing_periods, freq=4, return_NoObs=True):
        trailing_periods_obs = [self._trailing_periods[x] for x in trailing_periods]
        result = pd.DataFrame()
        for i in return_df.columns:
            returns = return_df[[i]]
            returns = returns.dropna()
            if len(trailing_periods_obs) == 1 and trailing_periods_obs[0] != 'Incep' and len(returns) < trailing_periods_obs[0]:
                ann_return = pd.DataFrame({returns.columns.name: [None],
                                       'AnnRor': [None],
                                       'NoObs': [None]})
                if return_NoObs:
            ...
            for period in trailing_periods:
                ...
                if trailing_period != "Incep":
                    if len(returns) < trailing_period:
                        ...
                        start_date = (
                                self._report_date + relativedelta(months=-3 * (trailing_period),
                                                                  days=1)
                        )
                        return_sub = returns.loc[start_date:self._report_date]
                else:
                    return_sub = returns
                if len(return_sub) <= 4:
    ...
    def calc_multiple(self, cf, group_cols=None, type='Gross'):
        ...
        if group_cols is None:
            multiple = cf[cf.TransactionType.isin(['Distributions', 'Net Asset Value'])].BaseAmount.sum() / abs(cf[
                cf.TransactionType.isin(['Contributions'])].BaseAmount.sum())
        else:
            multiple = cf[cf.TransactionType.isin(['Distributions', 'Net Asset Value'])].groupby(group_cols).BaseAmount.sum() / \
                       cf[cf.TransactionType.isin(['Contributions'])].groupby(
                group_cols).BaseAmount.sum().abs()
            multiple = multiple.reset_index().rename(columns={'BaseAmount': type + 'Multiple'})
            multiple['Name'] = multiple.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]), axis=1)
        return multiple
    ...
    def get_commitment(self, date, convert_to_usd):
        ...
        df.rename(columns={'OwnerName': 'Owner',
                           ...
                           'BaseAmount': 'Commitment'}, inplace=True)
        unfunded = df[(df.TransactionType == 'Unfunded Commitment Without Modification') & (df.TransactionDate == date)]
        funded = df[df.TransactionType != 'Unfunded Commitment Without Modification']
        funded = funded[['Owner', 'Investment', 'Commitment']].groupby(['Owner', 'Investment']).sum().reset_index()

        rslt = pd.concat([unfunded[['Owner', 'Investment', 'Commitment']], funded])
    ...
    def get_ctrs(self, ctrs, trailing_periods):
        ...
        for trailing_period in trailing_periods:
            if trailing_period != 'Incep':
                start_date = (
                                self._report_date + relativedelta(months=-3*(self._trailing_periods.get(trailing_period)),
                                                                  days=1)
                            )
                ctr = self.calc_ctr(ctrs.loc[start_date:self._report_date])
                ...
                ctr.AnnCtr = ctr[['Ctr', 'NoObs']].apply(lambda row: (1 + row['Ctr']) ** (safe_division(4, row['NoObs'])) - 1, axis=1)
    ...
    def get_ror_ctr(self, df, group_cols, trailing_periods):
        rors = self.calc_tw_ror(df, group_cols=group_cols, return_support_data=True)
        if len(rors) == 0:
            return pd.DataFrame(columns=['Name', 'Period', 'AnnRor', 'Ctr'])

        try:
            rors.to_csv(f"C:/Tmp/ror data check {str(group_cols)} {str(self._portfolio)} {str(self._report_date)}.csv")
        except:
        ...
        ctr = self.get_ctrs(ctrs=rors.pivot_table(index="Date", columns="Name", values="Ctr").fillna(0),
                            trailing_periods=trailing_periods)

        result = ann_ror.merge(ctr, how='outer', left_on=['Name', 'Period'], right_on=['Name', 'Period'])
    ...
    def get_irr_df_rpt(self, df, list_to_iterate):
        irr_data = pd.concat([self.calc_irr(df, group_cols=i) for i in list_to_iterate])[['Name', 'GrossIrr']]
        irr_data['NoObs'] = 'Incep'

        return irr_data

    def get_ror_ctr_df_rpt(self, df, trailing_periods, list_to_iterate):
        ror_ctr_df = pd.concat([self.get_ror_ctr(df, group_cols=i, trailing_periods=trailing_periods) for i in list_to_iterate])[['Name', 'AnnRor', 'Ctr', 'Period', 'group_cols']]
        result = pd.DataFrame()
        for i in ror_ctr_df.Period.unique():
            ...
            for x in sub.group_cols.unique():
                ...
                ctr_total = sub[sub.Name == self._portfolio_ticker].AnnRor.squeeze() / sum(subb[~subb.Ctr.isnull()].Ctr)
                subb['Ctr'] = subb.Ctr * ctr_total.squeeze()
                result = pd.concat([result, subb])[['Name', 'AnnRor', 'Ctr', 'Period']]
        return result
    ...
    def get_multiple_df_rpt(self, df, list_to_iterate):
        ...
        multiple_df = pd.concat([self.calc_multiple(df, group_cols=i) for i in list_to_iterate])[['Name', 'GrossMultiple']]
    ...
    def calc_wtd_holding_period(self, df, group_cols):
        df['PctEquity'] = df.BaseAmount / df.groupby(group_cols).BaseAmount.transform('sum')
    ...
    def recurse_down(self, df: pd.DataFrame,
                     group_by_list: List[str],
                     depth: int,
                     atomic_units: List[str]) -> pd.DataFrame:
        ...
        if df.shape[0] > 0:
            ...
            for name, group in current_grouping_struct:
                ...
                simple_frame = pd.DataFrame({"Name": [name], "Layer": [depth], "GrossIrr": [self.calc_irr(group)]})
                ...
                if group.shape[0] > 0 and depth < len(group_by_list) - 1:
                    children_data_frame = self.recurse_down(df=group,
    ...
    def recurse_down_order(self, df: pd.DataFrame,
                     group_by_list: List[str],
                     depth: int,
                     counter: int) -> pd.DataFrame:
        # if counter == 0:
        #     counter = 0
        #     last_depth = 0
        # else:
        ...
        final_df_cache = []
        if df.shape[0] > 0:
            current_grouping_struct = df.groupby(group_by_list[depth], sort=False)
            for name, group in current_grouping_struct:
                if depth == 4:
                    print('smart guy')
                simple_frame = pd.DataFrame({"DisplayName": [name], "Layer": [depth],
                                             "Name": [group.reset_index()['Group' + str(depth)][0]],
                                             "Description": [group.reset_index()['Group' + str(depth-1 if depth != 0 else 0)][0]],
                                             "Counter": [counter]})
                final_df_cache.append(simple_frame)
                ...
                reset = depth + 1
                if group.shape[0] > 0 and depth < len(group_by_list) - 1:
                    children_data_frame, counter = self.recurse_down_order(df=group,
                                                            group_by_list=group_by_list,
                                                            depth=reset,
                                                            counter=counter - 1)
                    if children_data_frame is not None:
                    ...
                    if (len(final_df_cache) == len(current_grouping_struct)) & (depth < len(group_by_list)):
                        counter = counter - 1
                    else:
    ...
    def calc_duration(self, discount_df, group_cols):
        ...
        df = discount_df.groupby(group_cols_extended).Discounted.sum().reset_index()
        df['DateInt'] = [i.toordinal() for i in df.Date]
        df['DurationCtr'] = df.DateInt * df.Discounted

        inflows = df[df.Type == 'Capital Call']
        inflows = (inflows.groupby(group_cols).DurationCtr.sum() / inflows.groupby(group_cols).Discounted.sum()).reset_index()
        inflows['avg_inflow_date'] = inflows[0].apply(convert_back_to_date)

        outflows = df[df.Type.isin(['Distribution', 'Value'])]
        outflows = (outflows.groupby(group_cols).DurationCtr.sum() / outflows.groupby(
            group_cols).Discounted.sum()).reset_index()
        ...
        df_dates['DateDiff'] = (df_dates.avg_outflow_date - df_dates.avg_inflow_date)
        df_dates['Duration'] = df_dates.DateDiff.apply(convert_timedelta_to_years)

        df_dates['Name'] = df_dates.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]), axis=1)
        return df_dates

    def get_max_or_min_df_rpt(self, df, list_to_iterate, min=True):
        rslt = pd.concat([self.calc_max_or_min(df, group_cols=i, min=min) for i in list_to_iterate])
    ...
    def get_holding_periods_rpt(self, df, discount_df, list_to_iterate):
        max_nav_date = df[df.TransactionType == 'Net Asset Value'].groupby(['Name']).TransactionDate.max().reset_index().\
            rename(columns={'TransactionDate': 'MaxNavDate'})

        min_cf_date = df[self._attributes_needed + ['TransactionDate', 'Portfolio']].groupby(self._attributes_needed + ['Portfolio']).min().reset_index()
        date_df = min_cf_date.merge(max_nav_date, how='outer')
        date_df['HoldingPeriod'] = (date_df.MaxNavDate - date_df.TransactionDate) / pd.Timedelta('365 days')

        equity_invested = df[df.TransactionType == 'Contribution'].\
            groupby(self._attributes_needed + ['Portfolio']).BaseAmount.sum().abs().reset_index()

        holding_period_df = date_df.merge(equity_invested[['Name', 'BaseAmount']], how='left', left_on='Name', right_on='Name')

        rslt = pd.concat([self.calc_duration(discount_df, group_cols=i) for i in list_to_iterate])
    ...
    def get_sum_df_rpt(self, df, list_to_iterate):
        sum_df = pd.concat([self.calc_sum(df, group_cols=i) for i in list_to_iterate])
        sum_df['NoObs'] = 'Incep'
        return sum_df
    ...
    def get_investment_sector_benchmark(self, df):
        ...
        def my_dao_operation(dao, params):
            raw = f"select Ticker, Date, PxLast from analyticsdata.FactorReturns" \
        ...
        index_prices_rslt = index_prices.pivot(index='Date', columns='Ticker', values='PxLast').reset_index()
    ...
    def get_pct_gain_rpt(self, df, list_to_iterate):
        # add if we want
        dollar_gain = df[
            df.TransactionType.isin(['Distributions', 'Net Asset Value'])].groupby(
            self._attributes_needed + ['TransactionType', 'Portfolio']).BaseAmount.sum().reset_index()
        tmp = self.get_sum_df_rpt(dollar_gain, list_to_iterate)

    def format_and_get_bmark_ks_pme(self, df):
        # prep data
        fund_cf = df[self._attributes_needed +
        ...
        # get index prices
        fund_cf_bmark, index_prices = self.get_investment_sector_benchmark(fund_cf)
    ...
    def get_fv_cashflow_df(self, fund_df, fund_cf, index_prices):
        ...
        for idx in range(0, len(fund_df)):
            ...
            if len(single_fund.TransactionDate.unique()) < 2:
                continue
            single_fund_group_sum = single_fund.groupby(
                ['Name', 'TransactionDate', 'TransactionType']).sum().reset_index()
            max_nav_date = single_fund_group_sum[single_fund_group_sum.TransactionType == 'Value'].TransactionDate.max()
            total_nav = single_fund_group_sum[single_fund_group_sum.TransactionType == 'Value'].BaseAmount.sum()
            single_fund_group_sum = pd.concat([single_fund_group_sum[single_fund_group_sum.TransactionType != 'Value'],
                                               pd.DataFrame({'Name': single_fund_group_sum.Name.unique(),
                                                             'TransactionDate': [max_nav_date],
                                                             'TransactionType': 'Value',
                                                             'BaseAmount': total_nav})])
            fund_specific_index = index_prices[['Date', fund_df.BenchmarkTicker[idx]]]

            assert len(fund_specific_index.columns) == 2
            assert len(single_fund_group_sum.Name.unique()) == 1
            assert len(single_fund_group_sum[single_fund_group_sum.TransactionType == 'Value']) == 1
            assert single_fund_group_sum[
                       single_fund_group_sum.TransactionType == 'Value'].TransactionDate.max() == single_fund.TransactionDate.max()

            fv_cashflows = PmeModel().KS_PME(dates_cashflows=single_fund_group_sum["TransactionDate"],
                                             ...
                                             auto_NAV=False)
            index_value = \
            fund_specific_index[fund_specific_index.Date == PmeModel().nearest(fund_specific_index.Date, max_nav_date)][
                fund_df.BenchmarkTicker[idx]].iloc[0]
            discounted_NAV = (total_nav / index_value.squeeze())
            fv_cashflows_df = pd.concat([fv_cashflows_df,
                                         pd.DataFrame({'Name': single_fund_group_sum.Name.unique(),
    ...
    def get_ks_pme_rpt(self, df, nav_df, list_to_iterate):
        ...
        fund_df = fund_cf[self._attributes_needed + ['BenchmarkTicker']].drop_duplicates().reset_index(drop=True)
        result = pd.DataFrame()
        for trailing_period in list(self._trailing_periods.keys()):
            ...
            if trailing_period != 'Incep':
                start_date = self._report_date + relativedelta(
                    months=(self._trailing_periods.get(trailing_period) * -3),
                    days=1)
                starting_investment = nav_df[
                    nav_df.TransactionDate == pd.to_datetime(start_date + relativedelta(days=-1))]
                # starting_investment = starting_investment.groupby(group_cols).BaseAmount.sum().reset_index()
                starting_investment.BaseAmount = starting_investment.BaseAmount * -1
                starting_investment['Date'] = pd.to_datetime(start_date)
                starting_investment['TransactionType'] = 'Capital Call'

                fund_cf_filtered = fund_cf[fund_cf.TransactionDate >= pd.to_datetime(start_date)]
                fund_cf_filtered = pd.concat([starting_investment, fund_cf_filtered])
            else:
                fund_cf_filtered = fund_cf
                starting_investment = None

            fv_cashflows_df = self.get_fv_cashflow_df(fund_df=fund_df,
                                                      fund_cf=fund_cf_filtered,
                                                      index_prices=index_prices)
            if len(fv_cashflows_df) == 0:
                continue
            fv_cashflows_df_with_attrib = fv_cashflows_df.merge(df[self._attributes_needed + ['Portfolio']].drop_duplicates(),
                                                                how='left',
                                                                left_on='Name',
                                                                right_on='Name')
            ...
            for group_cols in list_to_iterate:
                if starting_investment is not None:
                    ...
                    full_period_groups = list(
                        starting_investment.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]), axis=1))

                    fv_cashflows_df_with_attrib = fv_cashflows_df_with_attrib[
                        fv_cashflows_df_with_attrib.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]),
                                                          axis=1).isin(
                            full_period_groups)]

                rslt = fv_cashflows_df_with_attrib.groupby(group_cols).sum().reset_index()
                rslt['KsPme'] = (rslt.sum_fv_distributions + rslt.discounted_nav) / rslt.sum_fv_calls
                rslt['Name'] = rslt.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]), axis=1)
    ...
    def get_alpha_discount_table(self, fund_df, fund_cf, index_prices):
        ...
        for idx in range(0, len(fund_df)):
            ...
            discount_table = PmeModel().discount_table(dates_cashflows=single_fund_group_sum["TransactionDate"],
                                             cashflows=single_fund_group_sum["BaseAmount"],
                                             cashflows_type=single_fund_group_sum["TransactionType"],
                                             dates_index=fund_specific_index.iloc[:, 0],
                                             index=fund_specific_index.iloc[:, 1])
            ...
            discount_table_rslt = pd.concat([discount_table_rslt, discount_table])
    ...
    def get_direct_alpha_rpt(self, df, nav_df, list_to_iterate):
        ...
        discount_df = self.get_alpha_discount_table(fund_df=fund_df, fund_cf=fund_cf, index_prices=index_prices)
        ...
        for group_cols in list_to_iterate:
            for trailing_period in list(self._trailing_periods.keys()):
                ...
                if trailing_period != 'Incep':
                    start_date = self._report_date + relativedelta(months=(self._trailing_periods.get(trailing_period) * -3),
                                                                   days=1)
                    starting_investment = nav_df[nav_df.TransactionDate == pd.to_datetime(start_date + relativedelta(days=-1))]
                    ...
                    index_start_value = \
                    index_prices[index_prices.Date == PmeModel().nearest(index_prices.Date, start_date)].iloc[0]

                    index_end_value = \
                    index_prices[index_prices.Date == PmeModel().nearest(index_prices.Date, self._report_date)].iloc[0]

                    initial_fv_factor = index_end_value[1] / index_start_value[1]
                    starting_investment['Discounted'] = starting_investment.BaseAmount * initial_fv_factor

                    grouped_filtered = discount_df[discount_df.Date >= pd.to_datetime(start_date)]
                    grouped_filtered = pd.concat([starting_investment, grouped_filtered])

                else:
                    grouped_filtered = discount_df.copy()
                    starting_investment = None

                discount_df_with_attrib = grouped_filtered[['Name', 'Date', 'Discounted']].merge(df[
                    self._attributes_needed + ['Portfolio']].drop_duplicates(),
                                                            how='left',
                                                            left_on='Name',
                                                            right_on='Name')
                ...
                if starting_investment is not None:
                    ...
                    discount_df_with_attrib = discount_df_with_attrib[
                        discount_df_with_attrib.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]), axis=1).isin(
                            full_period_groups)]

                rslt = discount_df_with_attrib.groupby(group_cols)[["Date", "Discounted"]].apply(xirr, silent=True)
                rslt = rslt.reset_index().rename(columns={0: 'Irr'})
                rslt['DirectAlpha'] = np.log(1 + rslt.Irr)

                rslt['Name'] = rslt.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]), axis=1)
                rslt['Period'] = trailing_period
                rslt = rslt[['Name', 'DirectAlpha', 'Period']].drop_duplicates()
                result = pd.concat([result, rslt])[['Name', 'DirectAlpha', 'Period']]
    ...
    def convert_amt_to_usd(self, df):
        ...
        assert len(df[(df.TransactionDate.isin(fx_rates.Date)) & (df.BaseCurrency.isin(fx_rates.FromCurrcyCd))]) == len(df[df.BaseCurrency != 'USD'])

        df_fx = df.merge(fx_rates, how='left',
                         left_on=['BaseCurrency', 'TransactionDate'],
                         right_on=['FromCurrcyCd', 'Date'])
        assert len(df) == len(df_fx)
        df_fx.Multiplier = np.where(df_fx.Multiplier.isnull(), 1, df_fx.Multiplier)
    ...
    def pivot_trailing_period_df(self, df):
        ...
        df_melted = df_melted.pivot_table(index='Name', columns=['Period', 'variable'],
    ...
    def get_horizon_irr_df_rpt(self, df, nav_df, list_to_iterate):
        ...
        for trailing_period in list(self._trailing_periods.keys()):
            ...
            if trailing_period != 'Incep':
                ...
                fund_cf_filtered = df[df.TransactionDate >= pd.to_datetime(start_date)]
                fund_cf_filtered = pd.concat([starting_investment, fund_cf_filtered])
            else:
                fund_cf_filtered = df
                starting_investment = None
            ...
            for group_cols in list_to_iterate:
                if starting_investment is not None:
                    ...
                    fund_cf_filtered = fund_cf_filtered[
                        fund_cf_filtered.apply(lambda x: '_'.join([str(x[i]) for i in group_cols]),
                                                          axis=1).isin(
                            full_period_groups)]

                irr_data = self.calc_irr(fund_cf_filtered, group_cols=group_cols)[['Name', 'GrossIrr']]
    ...
    def get_horizon_tvpi_df_rpt(self, df, nav_df, list_to_iterate):
        ...
        for trailing_period in list(self._trailing_periods.keys()):
            ...
            for group_cols in list_to_iterate:
                ...
                multiple_df = self.calc_multiple(fund_cf_filtered, group_cols=group_cols)[['Name', 'GrossMultiple']]
    ...
    def get_burgiss_bmark(self):
        ...
        rslt = df.set_index('Measure').T.reindex(['Pooled', 'TopQuartile', 'Median', 'BottomQuartile'])[['PME - S&P 500 (TR)',
                                                                                                         'Direct Alpha - S&P 500 (TR)',
                                                                                                         'TVPI',
                                                                                                         'IRR',
                                                                                                         'TWR - ITD',
        ...
        rslt[rslt.columns[rslt.columns.str.contains('IRR')]] = rslt[rslt.columns[rslt.columns.str.contains('IRR')]] / 100
        rslt[rslt.columns[rslt.columns.str.contains('Alpha')]] = rslt[rslt.columns[rslt.columns.str.contains('Alpha')]] / 100
        return rslt
    ...
    def get_pe_only_portfolios(self):
        ...
        not_pe = portfolios[portfolios.DealPredominantAssetClass != 'Private Equity'].PortfolioReportingName
        result = portfolios[~portfolios.PortfolioReportingName.isin(not_pe)].drop(columns='DealPredominantAssetClass').drop_duplicates()
        return result
    ...
    def get_twror_by_industry_rpt(self, owner, trailing_periods, list_to_iterate, convert_to_usd=False):
        if self._mgr_df is not None:
            self._mgr_df = self._mgr_df[self._mgr_df.HoldingName != 'Peak Rock Capital Credit Fund II LP']
            holdings_filter = self._mgr_df[
                self._mgr_df.InvestmentManagerName == owner].HoldingName.drop_duplicates().to_list()
            holdings_filter = [x for x in holdings_filter if 'Credit' not in x]
        else:
        ...
        if self._type_filter is not None:
            cf_irr = cf_irr[cf_irr.PredominantInvestmentType.isin(self._type_filter)]
        if self._asset_class_filter is not None:
            cf_irr = cf_irr[cf_irr.PredominantAssetClass.isin(self._asset_class_filter)]
        ...
        if self._asset_class_filter is not None:
            df = df[df.PredominantAssetClass.isin(self._asset_class_filter)]
        ...
        if len(cf_irr[(cf_irr.TransactionType == 'Net Asset Value') & (cf_irr.TransactionDate == self._report_date)]) == 0:
            print('No Current NAV')
            return

        direct_alpha, discount_df = self.get_direct_alpha_rpt(df=cf_irr, nav_df=nav_df, list_to_iterate=list_to_iterate)
        ks_pme = self.get_ks_pme_rpt(df=cf_irr, nav_df=nav_df, list_to_iterate=list_to_iterate)

        # TODO: make trailing_periods dynamic on YTD/QTD/TTM etc
        ror_ctr_df = self.get_ror_ctr_df_rpt(df, list_to_iterate=list_to_iterate, trailing_periods=trailing_periods)
        ...
        horizon_irr = self.get_horizon_irr_df_rpt(df=cf_irr, nav_df=nav_df, list_to_iterate=list_to_iterate)
        horizon_multiple = self.get_horizon_tvpi_df_rpt(df=cf_irr, nav_df=nav_df, list_to_iterate=list_to_iterate)
        ...
        if holdings_filter is not None:
            commitment = commitment[commitment.ReportingName.isin(holdings_filter)]
        if self._type_filter is not None:
            commitment = commitment[commitment.PredominantInvestmentType.isin(self._type_filter)]
        if self._asset_class_filter is not None:
            commitment = commitment[commitment.PredominantAssetClass.isin(self._asset_class_filter)]
        # commitment = commitment[commitment.PredominantInvestmentType == 'Primary Fund']

        commitment_df = self.get_sum_df_rpt(commitment, list_to_iterate)[['Name', 'Commitment', 'NoObs']]

        nav = cf_irr[cf_irr.TransactionType == 'Net Asset Value'].rename(columns={'BaseAmount': 'Nav'})
        nav_df = self.get_sum_df_rpt(nav, list_to_iterate)[['Name', 'Nav', 'NoObs']]

        discount_df_with_attrib = discount_df[['Name', 'Date', 'Discounted', 'Type']].merge(df[self._attributes_needed + ['Portfolio']].drop_duplicates(),
                         how='left',
                         left_on='Name',
                         right_on='Name')
        assert len(discount_df) == len(discount_df_with_attrib)

        holding_period_df, max_nav_date = self.get_holding_periods_rpt(cf_irr, discount_df_with_attrib, list_to_iterate)
        ...
        irr_melted = self.pivot_trailing_period_df(horizon_irr.rename(columns={'NoObs': 'Period'}))
        multiple_melted = self.pivot_trailing_period_df(horizon_multiple.rename(columns={'NoObs': 'Period'}))
        ...
        for group in range(0, len(list_to_iterate)):
            attrib['Group' + str(group)] = attrib.apply(lambda x: '_'.join([str(x[i]) for i in list_to_iterate[group]]), axis=1)
        attrib = attrib.sort_values([col_name for col_name in attrib.columns[attrib.columns.str.contains('Group')]])
        ...
        if 'PredominantRealizationTypeCategory' in attrib.columns:
            attrib.PredominantRealizationTypeCategory = attrib.PredominantRealizationTypeCategory.str.replace('Realized & Partially/Substantially Realized', 'Realized & Partial')
            attrib.PredominantRealizationTypeCategory = np.where(attrib.PredominantRealizationTypeCategory.isnull(),
                                                                 'Not Tagged',
        ...
        if 'PredominantInvestmentType' in attrib.columns:
            attrib['Order'] = np.select([
                ...
                (attrib.PredominantInvestmentType == 'Co-investment/Direct')
            ], [1, 2, 3])
            attrib = attrib.sort_values('Order')


        ordered_recursion = [item for sublist in list_to_iterate for item in sublist]
        ordered_recursion = [ordered_recursion[i] for i in range(len(ordered_recursion)) if i == ordered_recursion.index(ordered_recursion[i])]
        ordered_rpt_items, counter_df = self.recurse_down_order(attrib, group_by_list=ordered_recursion, depth=0, counter=0)
        rslt = reduce(lambda left, right: pd.merge(left, right, on=['Name'],
                                                   how='outer'),
                      [ordered_rpt_items, commitment_df, holding_period_df,
                       max_nav_date, nav_df, irr_melted, multiple_melted,
                       ks_pme_melted, direct_alpha_melted, ror_ctr_melted])
        rslt = rslt.sort_values(by=['Counter', 'Commitment'],
        ...
        columns = ['DisplayName', 'MaxNavDate', 'Duration', 'Commitment', 'Nav',
                   ('value', 'Incep', 'KsPme'), ('value', 'Incep', 'DirectAlpha'),
                    ('value', 'Incep', 'GrossMultiple'), ('value', 'Incep', 'GrossIrr'),
                   ('value', 'Incep', 'AnnRor'), ('value', 'Incep', 'Ctr'),
                   ('value', '5Y', 'KsPme'), ('value', '5Y', 'DirectAlpha'),
                   ('value', '5Y', 'GrossMultiple'), ('value', '5Y', 'GrossIrr'),
                   ('value', '5Y', 'AnnRor'), ('value', '5Y', 'Ctr'),
                   ('value', '3Y', 'KsPme'), ('value', '3Y', 'DirectAlpha'),
                   ('value', '3Y', 'GrossMultiple'), ('value', '3Y', 'GrossIrr'),
                   ('value', '3Y', 'AnnRor'), ('value', '3Y', 'Ctr'),
                   ('value', 'TTM', 'AnnRor'), ('value', 'TTM', 'Ctr'),
                   ('value', 'QTD', 'AnnRor'), ('value', 'QTD', 'Ctr')]
        ...
        input_data = {
            'Data': rslt,
            'FormatType': ordered_rpt_items[ordered_rpt_items.Layer == 1][['DisplayName']].drop_duplicates(),
            'FormatSector': ordered_rpt_items[ordered_rpt_items.Layer == 2][['DisplayName']].drop_duplicates(),
            'GroupThree': ordered_rpt_items[ordered_rpt_items.Layer == 3][['DisplayName']].drop_duplicates()
        }

        return input_data

    def generate_portfolio_report(self, convert_to_usd=False):
        input_data = self.get_twror_by_industry_rpt(owner=self._portfolio_ticker,
        ...
        as_of_date = dt.datetime.combine(self._report_date + MonthEnd(0), dt.datetime.min.time())
        report_json = {'Date': pd.DataFrame({'Date': [as_of_date]}),
                       'PortfolioName': pd.DataFrame({'PortfolioName': [self._portfolio]},),
                       'benchmark_df': benchmark_df}
        report_json.update(input_data)

        # hide cols
        data = input_data.get('Data').copy()
        if data[data.DisplayName == self._portfolio_ticker][('value', '3Y', 'AnnRor')].squeeze() is None:
            hide_cols = ['M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X']
        elif data[data.DisplayName == self._portfolio_ticker][('value', '5Y', 'AnnRor')].squeeze() is None:
        ...
        with Scenario(as_of_date=as_of_date).context():
            InvestmentsReportRunner().execute(
                data=report_json,
                print_areas={'Industry Breakdown': "B1:AB" + str(len(report_json.get('Data')) + 20)},
                hide_cols=hide_cols,
    ...
    def set_manager_rpt_settings(self, manager_name):
        ...
        os_list = str(list(self._portfolios.OperationalSeriesTicker.unique()))
    ...
    def run(self, **kwargs):
        ...
        portfolio_list = list(self._portfolios.PortfolioReportingName.drop_duplicates())
        primaries_os = self.get_portfolio_os_investment_type('Primary Fund')
        portfolio_list = ['Trive Capital Management, LLC',
        ...
        for x in range(0, len(portfolio_list)):
            try:
                ...
                print(f"{portfolio} - {x} out of {len(portfolio_list)}; {round(x/len(portfolio_list),2)*100}%")
                ...
                # os_list = str(list(portfolios[portfolios.PortfolioReportingName == portfolio].OperationalSeriesTicker))
                # os_list = str(list(portfolios.OperationalSeriesTicker.unique()))
...
if __name__ == "__main__":
    ...
    }, as_of_date=report_date).context():

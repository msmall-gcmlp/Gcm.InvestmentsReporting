...
class PmeModel(ReportingRunnerBase):
    def __init__(self, exclude_net=False):
        super().__init__(dao=Scenario.get_attribute("dao"))
        self._helpers = RunPeracs()
        self._runner = Scenario.get_attribute("dao")
        self._report_date = Scenario.get_attribute("as_of_date")
        self._exclude_net = exclude_net
        self._gross_file_name = 'C:/Tmp/peracs/INTERNAL-USE-Gross-Par-All - One Rock v35 202209 v01 GR - 2023.02.09 13.09.38.xlsx'
        self._net_file_name = 'C:/Tmp/peracs/INTERNAL-USE-Net-Par-All - One Rock v35 202209 v01 GR - 2023.02.09 13.09.38.xlsx'

    # Helper functions
    def nearest(self, series, lookup, debug=False):
        if debug == True:
    ...
    def xnpv(self, rate, values, dates):
        ...
        if rate <= -1.0:
            return float('inf')
        datesx = pd.to_datetime(dates).apply(
            lambda x: date(x.year, x.month, x.day))  # Just in case conversion to native datime date
    ...
    def TVM(self, value, value_date, money_date, discount_rate):
        ''' Calculates the discounted value of money to date money_date (i.e. either PV or FV depending on date)
        '''
        time_delta = ((pd.to_datetime(money_date) - pd.to_datetime(value_date)) / np.timedelta64(1, 'D')).astype(
            int) / 365
        return value * (1 + discount_rate) ** time_delta

    def xirr2(self, values, dates):
        datesx = pd.to_datetime(dates).apply(
            lambda x: date(x.year, x.month, x.day))  # Just in case conversion to native datime date
    ...
    def discount_table(self, dates_cashflows, cashflows, cashflows_type, dates_index, index, NAV_scaling=1):
        ...
        # Get NAV
        if (df_cf[(df_cf['Type'] == 'Value') & (
                df_cf['Amount'] == 0)].empty):  # Checks if liquidated by looking at 0 valuations
            NAV_record = df_cf[df_cf['Type'] == 'Value'].sort_values("Date", ascending=False).head(1).copy()
            NAV_date = NAV_record["Date"].iloc[0]
            NAV_index_value = index[_dates_index == self.nearest(_dates_index, NAV_date)].iloc[0]
            df_cf['Status'] = 'Active'
        else:  # Not liquidated
            NAV_record = df_cf[df_cf['Type'] != 'Value'].sort_values("Date", ascending=False).head(1).copy()
        ...
        for idx, cf in df_cf.iterrows():
            ...
            if cf["Type"] == "Distribution":
                df_cf.loc[idx, 'Discounted'] = cf["Amount"] * (NAV_index_value / index_value)
        ...
        df_cf.loc[(df_cf['Date'] == NAV_date) & (df_cf['Type'] == 'Value'), 'Discounted'] = NAV_record['Amount'].iloc[
                                                                                                0] * NAV_scaling
        df_cf.loc[(df_cf['Date'] == NAV_date) & (df_cf['Type'] == 'Value'), 'Pre-Discounted'] = \
    ...
    # KS-PME
    def KS_PME(self, dates_cashflows, cashflows, cashflows_type, dates_index, index, NAV_scaling=1, auto_NAV=True):
        ...
        for idx, cf in df_cf.iterrows():
            # Let us find the closest index value to the current date
            index_value = index[_dates_index == self.nearest(_dates_index, cf["Date"])].iloc[0]
            if cf["Type"] == "Distribution":
        ...
        if auto_NAV == True:
            ...
            index_value = index[_dates_index == self.nearest(_dates_index, NAV_record["Date"].iloc[0])].iloc[0]
    ...
    # Direct Alpha
    def Direct_Alpha_PME(self, dates_cashflows, cashflows, cashflows_type, dates_index, index, NAV_scaling=1):
        ...
        df_cf = self.discount_table(dates_cashflows, cashflows, cashflows_type, dates_index, index, NAV_scaling=NAV_scaling)
        # Now let us calculate the IRR
        # irr = self.xirr2(df_cf['Discounted'], pd.to_datetime(df_cf['Date']))
        irr = xirr(df_cf[['Date', 'Discounted']])
        direct_alpha = np.log(1 + irr)
        return direct_alpha

    # PME+
    def PME_PLUS(self, dates_cashflows, cashflows, cashflows_type, dates_index, index, return_alpha=1, NAV_scaling=1):
        ...
        pme_plus = self.xirr2(df_cf['PME_PLUS IRR'], pd.to_datetime(df_cf['Date']))
        irr = self.xirr2(df_cf['Pre-Discounted'], pd.to_datetime(df_cf['Date']))
    ...
    # MIRR
    def MIRR(self, dates_cashflows, cashflows, cashflows_type, reinvestment_rate, financing_rate, first_date=0,
             NAV_scaling=1):
        ...
        for idx, cf in df_cf.iterrows():
            if cf["Type"] == "Distribution":
                sum_fv_dist = sum_fv_dist + self.TVM(cf['Amount'], cf['Date'], end_date, reinvestment_rate)
            elif cf["Type"] == "Capital Call":
                sum_pv_calls = sum_pv_calls + self.TVM(-cf['Amount'], cf['Date'], start_date, financing_rate)
                # Grab the NAV if available
        if (df_cf[(df_cf['Type'] == 'Value') & (
                df_cf['Amount'] == 0)].empty):  # Checks if liquidated by looking at 0 valuations
            NAV_record = \
            df_cf[df_cf['Type'] == 'Value'].sort_values("Date", ascending=False).head(1).copy()['Amount'].iloc[0]
        else:
            NAV_record = 0
            # Get the Time delta (n)
        time_delta = ((pd.to_datetime(end_date) - pd.to_datetime(start_date)) / np.timedelta64(1, 'D')).astype(
            int) / 365
    ...
    # Adjust Beta
    def BETA_ADJ_INDEX(self, dates_index, index, rf_date, rf_rate, beta):
        ...
        for idx, record in df_index.iterrows():
            ...
            closest_rf = float(df_rf.loc[df_rf["Date"] == self.nearest(pd.to_datetime(df_rf["Date"]),
                                                                  pd.to_datetime(record['Date']), 0), 'Value'])
            try:
            ...
            if (record['Index'] != 100 and int(idx) != 0):
                df_index.loc[idx, 'Index_beta_adjusted'] = (1 + df_index.loc[idx, 'Adjusted_delta']) * df_index.loc[
                    int(idx) - 1, 'Index_beta_adjusted']
    ...
    def run(self, **kwargs):
        manager_attrib, fund_attrib, deal_attrib, deal_cf = self._helpers.get_attrib_and_cfs()

        fund_cfs = deal_cf.groupby(['CashflowDate', 'FundName', 'CashflowType']).sum().reset_index()

        cf_data = fund_cfs.rename(columns={'FundName': 'Fund ID',
                                            'CashflowDate': 'Transaction Date',
                                            'CashflowType': 'Transaction Category',
                                            'Cf': 'Transaction Amount'})
        cf_data['Transaction Category'] = np.where(cf_data['Transaction Category']=='R', 'Value', cf_data['Transaction Category'])
        ticker = "SPXT Index"
        local_index = Factor(
                tickers=[ticker]
            ).get_returns(
                start_date=dt.date(2010, 1, 1), # change
                end_date=self._report_date + MonthEnd(1),
                fill_na=True,
            ).reset_index()
        for idx in local_index.index:
            if idx == local_index.index[0]:
                local_index.loc[idx, 'Value'] = 100
            else:
                local_index.loc[idx, 'Value'] = local_index.shift(1).loc[idx,'Value'] * (1 + local_index.loc[idx, ticker])
        local_index = local_index[['Date', 'Value']]

        cf_funds = fund_attrib.reset_index()[['index', 'FundName']].rename(
            columns={'FundName': 'Fund ID'}
        )
        for idx, fund in cf_funds.iterrows():
            fund_cf = cf_data[cf_data['Fund ID'] == fund['Fund ID']].copy()
            cf_funds.loc[idx, 'local_KSPME'] = self.KS_PME(fund_cf["Transaction Date"], fund_cf["Transaction Amount"],
                                                            fund_cf["Transaction Category"], local_index.iloc[:, 0],
                                                            local_index.iloc[:, 1])
            cf_funds.loc[idx, 'DirectAlpha'] = self.Direct_Alpha_PME(fund_cf["Transaction Date"],
                                                                      fund_cf["Transaction Amount"],
                                                                      fund_cf["Transaction Category"],
                                                                      local_index.iloc[:, 0], local_index.iloc[:, 1])
            cf_funds.loc[idx, 'PME+'] = self.PME_PLUS(fund_cf["Transaction Date"], fund_cf["Transaction Amount"],
                                                       fund_cf["Transaction Category"], local_index.iloc[:, 0],
                                                       local_index.iloc[:, 1])

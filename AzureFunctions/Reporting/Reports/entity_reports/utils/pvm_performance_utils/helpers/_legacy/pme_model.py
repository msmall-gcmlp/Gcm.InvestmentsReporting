...
class PmeModel(ReportingRunnerBase):
    ...
    def xnpv(self, rate, values, dates):
        ...
        )  # Just in case conversion to native datime date
        d0 = datesx[0]  # or min(dates)
        return sum(
            [
                vi / (1.0 + rate) ** ((di - d0).days / 365.0)
                for vi, di in zip(values, datesx)
            ]
        )
    ...
    def xirr(self, values, dates):
        ...
        try:
            return scipy.optimize.newton(
                lambda r: self.xnpv(r, values, dates), 0.0
            )
        except RuntimeError:  # Failed to converge?
            return scipy.optimize.brentq(
                lambda r: self.xnpv(r, values, dates), -1.0, 1e10
            )
    ...
    def TVM(self, value, value_date, money_date, discount_rate):
        ...
        time_delta = (
            (pd.to_datetime(money_date) - pd.to_datetime(value_date))
            / np.timedelta64(1, "D")
        ).astype(int) / 365
    ...
    def xirr2(self, values, dates):
        ...
        years = [
            (ta[0] - transactions[0][0]).days / 365.0
    ...
    def Direct_Alpha_PME(
        ...
        df_cf = self.discount_table(
            dates_cashflows,
            cashflows,
            cashflows_type,
            dates_index,
            index,
            NAV_scaling=NAV_scaling,
        )
    ...
    def PME_PLUS(
        ...
        for idx, cf in df_cf.iterrows():
            if cf["Type"] == "Distribution":
                sum_fv_distributions = (
                    sum_fv_distributions
                    + abs(cf["Amount"]) * cf["FV_Factor"]
                )
            elif cf["Type"] == "Capital Call":
                sum_fv_calls = (
                    sum_fv_calls + abs(cf["Amount"]) * cf["FV_Factor"]
                )
        ...
        for idx, cf in df_cf.iterrows():
            if cf["Type"] == "Distribution":
                df_cf.loc[idx, "PME_PLUS IRR"] = (
                    abs(cf["Amount"]) * scaling_factor
                )
            elif cf["Type"] == "Capital Call":
                df_cf.loc[idx, "PME_PLUS IRR"] = -abs(cf["Amount"])

        pme_plus = self.xirr2(
            df_cf["PME_PLUS IRR"], pd.to_datetime(df_cf["Date"])
        )
        irr = self.xirr2(
            df_cf["Pre-Discounted"], pd.to_datetime(df_cf["Date"])
        )
    ...
    def MIRR(
        ...
        for idx, cf in df_cf.iterrows():
            if cf["Type"] == "Distribution":
                sum_fv_dist = sum_fv_dist + self.TVM(
                    cf["Amount"], cf["Date"], end_date, reinvestment_rate
                )
            elif cf["Type"] == "Capital Call":
                sum_pv_calls = sum_pv_calls + self.TVM(
                    -cf["Amount"], cf["Date"], start_date, financing_rate
                )
        ...
        return (
            ((sum_fv_dist + NAV_record) / sum_pv_calls) ** (1 / time_delta)
        ) - 1
    ...
    def BETA_ADJ_INDEX(self, dates_index, index, rf_date, rf_rate, beta):
        ...
        df_index["Delta_amount"] = (
            df_index["Index"] / df_index["Index"].shift(1)
        ) - 1
        df_index["Delta_time_days"] = (
            df_index["Date"] - df_index["Date"].shift(1)
        ) / np.timedelta64(1, "D")
        ...
        for idx, record in df_index.iterrows():
            ...
            adjusted_delta_amount = closest_rf_adj + beta * (
                record["Delta_amount"] - closest_rf_adj
            )
            ...
            if record["Index"] != 100 and int(idx) != 0:
                df_index.loc[idx, "Index_beta_adjusted"] = (
                    1 + df_index.loc[idx, "Adjusted_delta"]
                ) * df_index.loc[int(idx) - 1, "Index_beta_adjusted"]

        # clean up, produce new DF, and return it
        df_index_adjusted = pd.concat(
            [
                df_index["Date"],
                df_index["Index_beta_adjusted"],
                df_index["Index"],
            ],
            axis=1,
        )
    ...
    def run(self, **kwargs):
        (
            ...
            deal_attrib,
            deal_cf,
        ) = self._helpers.get_attrib_and_cfs()

        fund_cfs = (
            deal_cf.groupby(["CashflowDate", "FundName", "CashflowType"])
            .sum()
            .reset_index()
        )

        cf_data = fund_cfs.rename(
            columns={
                "FundName": "Fund ID",
        ...
        cf_data["Transaction Category"] = np.where(
            cf_data["Transaction Category"] == "R",
            "Value",
            cf_data["Transaction Category"],
        )
        ticker = "SPXT Index"
        local_index = (
            Factor(tickers=[ticker])
            .get_returns(
        ...
        for idx in local_index.index:
            if idx == local_index.index[0]:
                ...
                local_index.loc[idx, "Value"] = local_index.shift(1).loc[
                    idx, "Value"
                ] * (1 + local_index.loc[idx, ticker])
        ...
        for idx, fund in cf_funds.iterrows():
            ...
            cf_funds.loc[idx, "local_KSPME"] = self.KS_PME(
                fund_cf["Transaction Date"],
                fund_cf["Transaction Amount"],
                fund_cf["Transaction Category"],
                local_index.iloc[:, 0],
                local_index.iloc[:, 1],
            )
            cf_funds.loc[idx, "DirectAlpha"] = self.Direct_Alpha_PME(
                fund_cf["Transaction Date"],
                fund_cf["Transaction Amount"],
                fund_cf["Transaction Category"],
                local_index.iloc[:, 0],
            ...
            cf_funds.loc[idx, "PME+"] = self.PME_PLUS(
                fund_cf["Transaction Date"],
                fund_cf["Transaction Amount"],
                fund_cf["Transaction Category"],
                local_index.iloc[:, 0],

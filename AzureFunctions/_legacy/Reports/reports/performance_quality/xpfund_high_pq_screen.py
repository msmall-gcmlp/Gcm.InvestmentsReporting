...
from _legacy.Reports.reports.performance_quality.tmp import _xpfund_data_to_highlow_df,_5y_arb_xs_emm_percentiles
...
class XPfundHighPQScreen(ReportingRunnerBase):
    ...
    def _xpfund_pq_report(self):       
        ...
        inputs = PerformanceQualityHelper().download_inputs(location="raw/investmentsreporting/summarydata/xpfund_performance_quality", file_path="_firm_x_portfolio_fund2023-03-31.json")
        return inputs
    ...
    def generate_xpfund_high_pq_screen_data(self):
        with Scenario(dao=self._runner, as_of_date=self._as_of_date).context():
            ...
            y = self._get_xpfund_pq_report(runner=self._runner, as_of_date=date_q_minus_1)
            m=y["('AbsoluteReturnBenchmarkExcess', '3Y')"]
            #y['x']=y["('AbsoluteReturnBenchmarkExcess', '5Y')"]
            #y['5y_ptiles'] = y.x.rank(numeric_only=True, pct = True)
            #n=_5y_arb_xs_emm_percentiles(y)
            #g=y['5y_ptiles']
            z=_xpfund_data_to_highlow_df(y)

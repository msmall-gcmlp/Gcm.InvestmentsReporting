...
def _net_exposure_clean(df):
    ...
    df['net_exp_adj_latest'] = df[
        """('Latest', "('Equities', 'NetNotional')")"""
            ...
            """('Latest', "('Credit', 'NetNotional')")"""].fillna(0)
...
def _status_alloc(df, as_of_date):
    ...
    portfolio_allocation_status = portfolio_allocation_status[(portfolio_allocation_status['Acronym'] == 'GMSF') | 
                                                              (portfolio_allocation_status['Acronym'] == 'SPECTRUM')]

...
class SingleNameEquityExposureSummary(ReportingRunnerBase):
    ...
    def build_single_name_summary(self):
        ...
        portfolio_level = portfolio_level.groupby(['Acronym', 'Issuer', 'Sector'])['PortfolioNav'].sum().reset_index()
        portfolio_level.sort_values(['Acronym', 'PortfolioNav'], ascending=False, inplace=True)
        largest_portfolio_level = portfolio_level.groupby(['Acronym']).head(5).reset_index(drop=True)
        largest_portfolio_level = largest_portfolio_level[['Acronym', 'Issuer', 'Sector', 'PortfolioNav']]
        ...
        largest_firm_wide_level = largest_firm_wide_level.groupby(['Issuer', 'Sector', 'IssuerSum'])['InvestmentGroupName'].agg(
            ','.join).reset_index()
        largest_firm_wide_level = largest_firm_wide_level[['Issuer', 'InvestmentGroupName', 'Sector', 'IssuerSum']]
    ...
    def generate_single_name_summary_report(self):
        ...
        with Scenario(as_of_date=as_of_date).context():
            InvestmentsReportRunner().execute(
                data=input_data,
                template="SingleNameExposure_Template_Summary.xlsx",
                save=True,
                save_as_pdf=True,
                runner=self._runner,
                ...
                report_name="ARS Single Name Equity Exposure Summary",

...
class BaselReportData(ReportingRunnerBase):
    ...
    def aggregated_exposure(self):
        ...
        fund_exposure = fund_exposure[
            fund_exposure["InvestmentName"].isin(portfolio_allocation["InvestmentName"].to_list())
        ]
        ...
        for t1 in long_short_list:
            for t2 in metrics_list:
                temp = portfolio_exp_temp[
                    (portfolio_exp_temp["LongShort"] == t1) & (portfolio_exp_temp["metrics"] == t2)
                ]
                ...
                result_df = pd.DataFrame({"InvestmentName": [self._portfolio_name], "LongShort": [t1], "metrics": [t2]})

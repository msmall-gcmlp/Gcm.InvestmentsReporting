...
class BaselReport(ReportingRunnerBase):
    ...
    def get_as_of_date(self):
        header = pd.DataFrame({"asofdate": [self._as_of_date]})
        return header
    ...
    def get_exposure_by_category(self):
        ...
        for tab in mapping["Tab"].unique():
            if tab == "Equity":
                column_equity = ["metrics"] + mapping[
                    mapping["Tab"] == "Equity"
                ]["TemplateMapping"].values.tolist()
                data_equity = data.loc[:, column_equity]
                data_equity = data_equity[
                    data_equity["metrics"] == "Market_Over_NAV"
                ]
            ...
            if tab == "Derivative":
                columns_deriv = ["LongShort", "metrics"] + mapping[
                    mapping["Tab"] == "Derivative"
                ]["TemplateMapping"].values.tolist()
                data_deriv = data.loc[:, columns_deriv]
                data_deriv_long = data_deriv[
                    data_deriv["LongShort"] == "Long"
                ].sort_values(by="metrics", ascending=False)
                data_deriv_short = data_deriv[
                    data_deriv["LongShort"] == "Short"
                ].sort_values(by="metrics", ascending=False)
                ...
                for x in mapping[mapping["Tab"] == "Derivative"][
                    "TemplateMapping"
                ].values.tolist():
                    dic[x] = pd.DataFrame(
                        [data_deriv_long[x]],
                ...
                data_deriv_long = {
                    k + "_Long": v for k, v in data_deriv_long.items()
                }
                data_deriv_short = {
                    k + "_Short": v for k, v in data_deriv_short.items()
                }
                data_deriv_long.update(data_deriv_short)
                for x, y in data_deriv_long.items():
                    dic[x] = pd.DataFrame(
                        [y], columns=["Notional", "Market"]
                    )
            else:
                columns_repo = ["LongShort", "metrics"] + mapping[
                    mapping["Tab"].isin(
                        ["SFT (ReverseRepo)", "SFT (Repo)"]
                    )
                ]["TemplateMapping"].values.tolist()
                data_repo = data.loc[:, columns_repo]
                data_repo = data_repo[
                    data_repo["metrics"] == "Market_Over_NAV"
                ]
                data_repo_long = data_repo[
                    data_repo["LongShort"] == "Long"
                ].sort_values(by="metrics", ascending=False)
                data_repo_short = data_repo[
                    data_repo["LongShort"] == "Short"
                ].sort_values(by="metrics", ascending=False)
                ...
                for x in mapping[
                    ...
                    dic[x] = pd.DataFrame(
                        [data_repo_long[x]], columns=["Market"]
                    ) + pd.DataFrame(
                        [data_repo_short[x]], columns=["Market"]
                    )
        return dic

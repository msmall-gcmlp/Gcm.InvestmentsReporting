...
def copy_sheet_attributes(
    ...
    # Does not work (because of meta data in the attribute I think).
    ...
    for key, value in source_sheet.column_dimensions.items():
        ...
        # Excel actually groups multiple columns under 1 key.
        # Use the min max attribute to also group the columns in the targetSheet
        target_sheet.column_dimensions[key].max = copy(
            source_sheet.column_dimensions[key].max
        )
        # https://stackoverflow.com/questions/36417278/openpyxl-can-not-read-consecutive-hidden-columns
        # discussed the issue. Note that this is also the case for the width,
        # not onl;y the hidden property
...
def copy_cells(source_sheet, target_sheet):
    for r, row in enumerate(source_sheet.iter_rows()):
        for c, cell in enumerate(row):
            ...
            if (
                not isinstance(source_cell, openpyxl.cell.ReadOnlyCell)
                and source_cell.hyperlink
            ):
                target_cell._hyperlink = copy(source_cell.hyperlink)

            if (
                not isinstance(source_cell, openpyxl.cell.ReadOnlyCell)
                and source_cell.comment
            ):

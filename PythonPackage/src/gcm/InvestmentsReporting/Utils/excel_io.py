...
class ExcelIO(object):
    def write_dataframe_to_xl(self, wb, dataframe, template_sheet_name, first_cell, alignment=None, font=None):
        ...
        for r_idx, row in enumerate(rows, 0):
            for c_idx, value in enumerate(row, 0):
                ws.cell(row=int(start_row) + r_idx,
                        column=column_index_from_string(start_col) + c_idx,
                        value=value)
                if alignment is not None:
                    ws.cell(row=int(start_row) + r_idx,
                            column=column_index_from_string(start_col) + c_idx).alignment = alignment
                if font is not None:
                    ws.cell(row=int(start_row) + r_idx, column=column_index_from_string(start_col) + c_idx).font = font
        return wb
    ...
    def write_series_to_xl_long(self, wb, series, template_sheet_name, first_cell):
        ...
        for r_idx, value in enumerate(series):
            ws.cell(row=int(start_row) + r_idx, column=column_index, value=value)

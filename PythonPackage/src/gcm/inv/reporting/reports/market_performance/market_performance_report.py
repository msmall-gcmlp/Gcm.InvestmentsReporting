...
class MarketPerformanceReport(ReportingRunnerBase):
    def __init__(self, runner, as_of_date, interval, factor_daily_returns, price_change, prices, ticker_mapping):
    ...
    def _get_return_summary(self, returns, column_name, method):
        ...
        stats = [trailing_1D_return, trailing_week_return, mtd_return, qtd_return, ytd_return, trailing_1y_return]
        stats = [x.squeeze() for x in stats]
        summary = pd.DataFrame(
            {
                column_name[0]: [
                    round(x, 3) if isinstance(x, float) else " " for x in stats
                ]
            },
    ...
    def _out_under_preformance_vsbenchmark(self, returns, index, benchmark_return, method, period=PeriodicROR.MTD):
    ...
    def _general_summary(self, category, benchmark_returns=None, benchmarking=False):
        ...
        for column in combined_columns:
            transformation = self._ticker_mapping[
                self._ticker_mapping['Ticker'] == column].Transformation.values
            ...
            if benchmarking:
                out_under = self._out_under_preformance_vsbenchmark(input_returns,
                                                                   description,
                                                                   benchmark_returns,
            ...
            columns_transform = ['WTD', 'QTD', 'YTD', 'TTM', 'Max PTT (TTM)', 'Vol (Annlzd)']
            # to change the unit

            if unit_mult100[0]:
                function = lambda x: round((100 * x).astype('int')) if (x.name in columns_transform) else round(100 * x, 1) if (x.name in ['MTD','DTD']) else x
                agg_stat = agg_stat.apply(function)
            else:
                function = lambda x: round(x.astype('int')) if (x.name in columns_transform) else round(x, 1) if (x.name in ['MTD','DTD']) else x
                agg_stat = agg_stat.apply(function)

            #to add % sign
            if unit_in_prc[0]:
                function = lambda x: x.astype('str') + '%' if (x.name in columns_transform) else x.astype('str') + '%' if (x.name in ['MTD','DTD']) else x
                agg_stat = agg_stat.apply(function)
            else:
                function = lambda x: x.astype('str') if (x.name in columns_transform) else x.astype('str') if (x.name in ['MTD','DTD']) else x
                agg_stat = agg_stat.apply(function)
    ...
    def aggregated_stats(self, category):
        if category == 'SP_Sectors':
            ...
            base_summary = base_summary.reindex(
                columns=['index', 'Out_Under', 'empty', 'DTD', 'WTD', 'MTD', 'MTD1', 'Vol (Annlzd)',
                         'QTD', 'YTD','YTD1', 'TTM', 'Max PTT (TTM)'])

        elif category == 'L_S_Equity_Styles' or category == 'L_S_Equity_Industry_Factors':
            base_summary = self._general_summary(category)
            base_summary = pd.merge(self._ticker_mapping[['family', 'description']], base_summary.reset_index(),
                                    how='right', left_on ='description', right_on = 'index')
            ...
            base_summary['empty'] = ' '
            base_summary = base_summary.reindex(
                columns=['index', 'family', 'empty', 'DTD', 'WTD', 'MTD', 'MTD1', 'Vol (Annlzd)',
                         'QTD', 'YTD','YTD1', 'TTM', 'Max PTT (TTM)'])
            ...
            base_summary['empty1'] = ' '
            base_summary = base_summary.reindex(
                columns=['index', 'empty1', 'Last', 'DTD', 'WTD', 'MTD', 'MTD1', 'Vol (Annlzd)', 'QTD',
                         'YTD', 'YTD1', 'TTM', 'Max PTT (TTM)'])
        base_summary = base_summary.sort_values(by = ['MTD'], ascending = False)
        return base_summary
    ...
    def generate_market_performance_quality_report(self):
        ...
        credit = self.aggregated_stats('Credit')

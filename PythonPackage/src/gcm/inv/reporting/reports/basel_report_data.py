from gcm.inv.dataprovider.pub_dwh.allocation_query import get_pub_dwh_portfolio_holdings
...
class BaselReportData(ReportingRunnerBase):
    ...
    def portfolio_composition(self):
        allocation = get_pub_dwh_portfolio_holdings(self._runner,
                                                    self._as_of_date,
    ...
    def aggregated_exposure(self):
        ...
        portfolio_exp = pd.merge(fund_exposure, portfolio_allocation[['InvestmentName', 'PctNav']],
                                 how='left', on='InvestmentName')
        ...
        for t1 in long_short_list:
            for t2 in metrics_list:
                temp = portfolio_exp_temp[(portfolio_exp_temp['LongShort'] == t1) &
                                          (portfolio_exp_temp['metrics'] == t2)]
                new_temp = temp.drop(columns=['InvestmentName',
                                              'LongShort',
                                              'metrics'])
                prodsum = new_temp.agg(lambda x: sum(x.mul(new_temp['PctNav'],
                                       axis=0, fill_value=0)))
                prodsum_df = prodsum.to_frame()
                result_df = pd.DataFrame({'InvestmentName': [self._portfolio_name],
                                          'LongShort': [t1], 'metrics': [t2]})

...
from gcm.inv.reporting.reports.aggregate_performance_quality_report import (
    AggregatePerformanceQualityReport,
)
from gcm.inv.reporting.reports.performance_quality_peer_summary_report import (
    PerformanceQualityPeerSummaryReport,
)
from gcm.inv.reporting.reports.performance_quality_report_data import (
    PerformanceQualityReportData,
)
from gcm.inv.reporting.reports.performance_quality_report import (
...
class RunPerformanceQualityReports:
    ...
    def generate_fund_reports(self, fund_names):
        for fund in fund_names:
            ...
            perf_quality_report = PerformanceQualityReport(
                runner=self._runner,
                as_of_date=self._as_of_date,
                fund_name=fund,
            )
    ...
    def agg_perf_quality_by_portfolio(self, portfolio_acronyms=None):
        agg_perf_quality = AggregatePerformanceQualityReport(
            runner=self._runner,
            as_of_date=self._as_of_date,
...
if __name__ == "__main__":
    report_runner = RunPerformanceQualityReports(
    ...
    report_runner.generate_fund_reports(
        fund_names=[
            "Citadel",
